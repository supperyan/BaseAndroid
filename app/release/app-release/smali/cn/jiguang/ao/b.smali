.class public final Lcn/jiguang/ao/b;
.super Ljava/lang/Object;


# static fields
.field public static a:Lcn/jiguang/net/SSLTrustManager;


# direct methods
.method public static a(Landroid/content/Context;Ljava/lang/String;[BIII)Lcn/jiguang/ao/n;
    .registers 11

    const-string p5, "Exception - "

    const/4 v0, -0x2

    :try_start_3
    new-instance v1, Lcn/jiguang/net/HttpRequest;

    invoke-direct {v1, p1}, Lcn/jiguang/net/HttpRequest;-><init>(Ljava/lang/String;)V

    const/16 p1, 0x7530

    invoke-virtual {v1, p1}, Lcn/jiguang/net/HttpRequest;->setConnectTimeout(I)V

    invoke-virtual {v1, p1}, Lcn/jiguang/net/HttpRequest;->setReadTimeout(I)V

    const/4 p1, 0x1

    invoke-virtual {v1, p1}, Lcn/jiguang/net/HttpRequest;->setDoOutPut(Z)V

    invoke-virtual {v1, p1}, Lcn/jiguang/net/HttpRequest;->setDoInPut(Z)V

    const/4 v2, 0x0

    invoke-virtual {v1, v2}, Lcn/jiguang/net/HttpRequest;->setUseCaches(Z)V

    invoke-virtual {v1, p2}, Lcn/jiguang/net/HttpRequest;->setBody(Ljava/lang/Object;)V

    const-string v3, "Content-Length"

    array-length v4, p2

    invoke-static {v4}, Ljava/lang/String;->valueOf(I)Ljava/lang/String;

    move-result-object v4

    invoke-virtual {v1, v3, v4}, Lcn/jiguang/net/HttpRequest;->setRequestProperty(Ljava/lang/String;Ljava/lang/String;)V

    invoke-virtual {v1, p1}, Lcn/jiguang/net/HttpRequest;->setNeedRetryIfHttpsFailed(Z)V

    sget-object p1, Lcn/jiguang/ao/b;->a:Lcn/jiguang/net/SSLTrustManager;
    :try_end_2d
    .catchall {:try_start_3 .. :try_end_2d} :catchall_118
    .catch Ljava/lang/AssertionError; {:try_start_3 .. :try_end_2d} :catch_146
    .catch Ljava/lang/Exception; {:try_start_3 .. :try_end_2d} :catch_12f

    if-nez p1, :cond_3e

    :try_start_2f
    const-string p1, "-----BEGIN CERTIFICATE-----\\n-----END CERTIFICATE-----"

    invoke-static {p1}, Landroid/text/TextUtils;->isEmpty(Ljava/lang/CharSequence;)Z

    move-result v3

    if-nez v3, :cond_3e

    new-instance v3, Lcn/jiguang/net/SSLTrustManager;

    invoke-direct {v3, p1}, Lcn/jiguang/net/SSLTrustManager;-><init>(Ljava/lang/String;)V

    sput-object v3, Lcn/jiguang/ao/b;->a:Lcn/jiguang/net/SSLTrustManager;
    :try_end_3e
    .catchall {:try_start_2f .. :try_end_3e} :catchall_3e

    :catchall_3e
    :cond_3e
    :try_start_3e
    sget-object p1, Lcn/jiguang/ao/b;->a:Lcn/jiguang/net/SSLTrustManager;

    if-eqz p1, :cond_47

    sget-object p1, Lcn/jiguang/ao/b;->a:Lcn/jiguang/net/SSLTrustManager;

    invoke-virtual {v1, p1}, Lcn/jiguang/net/HttpRequest;->setSslTrustManager(Lcn/jiguang/net/SSLTrustManager;)V

    :cond_47
    const-string p1, "Accept"

    const-string v3, "application/jason"

    invoke-virtual {v1, p1, v3}, Lcn/jiguang/net/HttpRequest;->setRequestProperty(Ljava/lang/String;Ljava/lang/String;)V

    const-string p1, "Accept-Encoding"

    const-string v3, "gzip"

    invoke-virtual {v1, p1, v3}, Lcn/jiguang/net/HttpRequest;->setRequestProperty(Ljava/lang/String;Ljava/lang/String;)V

    const-string p1, "X-App-Key"

    invoke-static {p0}, Lcn/jiguang/am/a;->a(Landroid/content/Context;)Ljava/lang/String;

    move-result-object v3

    invoke-virtual {v1, p1, v3}, Lcn/jiguang/net/HttpRequest;->setRequestProperty(Ljava/lang/String;Ljava/lang/String;)V

    const-string p1, "Charset"

    const-string v3, "UTF-8"

    invoke-virtual {v1, p1, v3}, Lcn/jiguang/net/HttpRequest;->setRequestProperty(Ljava/lang/String;Ljava/lang/String;)V

    int-to-long v3, p3

    invoke-static {v3, v4}, Lcn/jiguang/bb/c;->a(J)Ljava/lang/String;

    move-result-object p1

    invoke-virtual {v1, v2}, Lcn/jiguang/net/HttpRequest;->setHaveRspData(Z)V

    invoke-virtual {v1, v2}, Lcn/jiguang/net/HttpRequest;->setRspDatazip(Z)V

    invoke-static {p2}, Lcn/jiguang/f/i;->a([B)Ljava/lang/String;

    move-result-object p2

    invoke-static {p0, p2, p1}, Lcn/jiguang/ao/k;->a(Landroid/content/Context;Ljava/lang/String;Ljava/lang/String;)Ljava/lang/String;

    move-result-object p1

    const-string p2, "Authorization"

    new-instance p3, Ljava/lang/StringBuilder;

    const-string v3, "Basic "

    invoke-direct {p3, v3}, Ljava/lang/StringBuilder;-><init>(Ljava/lang/String;)V

    invoke-virtual {p3, p1}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    invoke-virtual {p3}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object p1

    invoke-virtual {v1, p2, p1}, Lcn/jiguang/net/HttpRequest;->setRequestProperty(Ljava/lang/String;Ljava/lang/String;)V

    :cond_8b
    if-lez p4, :cond_110

    add-int/lit8 p4, p4, -0x1

    invoke-static {p0, v1}, Lcn/jiguang/net/HttpUtils;->httpPost(Landroid/content/Context;Lcn/jiguang/net/HttpRequest;)Lcn/jiguang/net/HttpResponse;

    move-result-object p1

    invoke-virtual {p1}, Lcn/jiguang/net/HttpResponse;->getResponseCode()I

    move-result p2

    const-string p3, "HttpHelper"

    new-instance v3, Ljava/lang/StringBuilder;

    const-string v4, "status code:"

    invoke-direct {v3, v4}, Ljava/lang/StringBuilder;-><init>(Ljava/lang/String;)V

    invoke-virtual {v3, p2}, Ljava/lang/StringBuilder;->append(I)Ljava/lang/StringBuilder;

    const-string v4, " retry left:"

    invoke-virtual {v3, v4}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    invoke-virtual {v3, p4}, Ljava/lang/StringBuilder;->append(I)Ljava/lang/StringBuilder;

    invoke-virtual {v3}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object v3

    invoke-static {p3, v3}, Lcn/jiguang/an/d;->c(Ljava/lang/String;Ljava/lang/String;)V

    const/16 p3, 0xc8

    if-eq p2, p3, :cond_106

    const/16 p3, 0x191

    if-eq p2, p3, :cond_fb

    const/16 p3, 0x194

    const/4 v3, -0x1

    if-eq p2, p3, :cond_f1

    const/16 p3, 0x19a

    if-eq p2, p3, :cond_f1

    const/16 p3, 0x1ad

    if-eq p2, p3, :cond_f1

    const/16 p3, 0x1f7

    if-eq p2, p3, :cond_e7

    const/16 p3, 0xbbd

    if-eq p2, p3, :cond_8b

    const/16 p0, 0x1f4

    if-lt p2, p0, :cond_dd

    new-instance p0, Lcn/jiguang/ao/n;

    invoke-virtual {p1}, Lcn/jiguang/net/HttpResponse;->getResponseBody()Ljava/lang/String;

    move-result-object p1

    invoke-direct {p0, v3, p1}, Lcn/jiguang/ao/n;-><init>(ILjava/lang/String;)V

    return-object p0

    :cond_dd
    new-instance p0, Lcn/jiguang/ao/n;

    invoke-virtual {p1}, Lcn/jiguang/net/HttpResponse;->getResponseBody()Ljava/lang/String;

    move-result-object p1

    invoke-direct {p0, v0, p1}, Lcn/jiguang/ao/n;-><init>(ILjava/lang/String;)V

    return-object p0

    :cond_e7
    new-instance p0, Lcn/jiguang/ao/n;

    invoke-virtual {p1}, Lcn/jiguang/net/HttpResponse;->getResponseBody()Ljava/lang/String;

    move-result-object p1

    invoke-direct {p0, v0, p1}, Lcn/jiguang/ao/n;-><init>(ILjava/lang/String;)V

    return-object p0

    :cond_f1
    new-instance p0, Lcn/jiguang/ao/n;

    invoke-virtual {p1}, Lcn/jiguang/net/HttpResponse;->getResponseBody()Ljava/lang/String;

    move-result-object p1

    invoke-direct {p0, v3, p1}, Lcn/jiguang/ao/n;-><init>(ILjava/lang/String;)V

    return-object p0

    :cond_fb
    new-instance p0, Lcn/jiguang/ao/n;

    const/4 p2, -0x3

    invoke-virtual {p1}, Lcn/jiguang/net/HttpResponse;->getResponseBody()Ljava/lang/String;

    move-result-object p1

    invoke-direct {p0, p2, p1}, Lcn/jiguang/ao/n;-><init>(ILjava/lang/String;)V

    return-object p0

    :cond_106
    new-instance p0, Lcn/jiguang/ao/n;

    invoke-virtual {p1}, Lcn/jiguang/net/HttpResponse;->getResponseBody()Ljava/lang/String;

    move-result-object p1

    invoke-direct {p0, v2, p1}, Lcn/jiguang/ao/n;-><init>(ILjava/lang/String;)V
    :try_end_10f
    .catchall {:try_start_3e .. :try_end_10f} :catchall_118
    .catch Ljava/lang/AssertionError; {:try_start_3e .. :try_end_10f} :catch_146
    .catch Ljava/lang/Exception; {:try_start_3e .. :try_end_10f} :catch_12f

    return-object p0

    :cond_110
    new-instance p0, Lcn/jiguang/ao/n;

    const-string p1, "Failed - retry enough"

    invoke-direct {p0, v0, p1}, Lcn/jiguang/ao/n;-><init>(ILjava/lang/String;)V

    return-object p0

    :catchall_118
    move-exception p0

    new-instance p1, Lcn/jiguang/ao/n;

    new-instance p2, Ljava/lang/StringBuilder;

    invoke-direct {p2, p5}, Ljava/lang/StringBuilder;-><init>(Ljava/lang/String;)V

    invoke-virtual {p0}, Ljava/lang/Throwable;->getMessage()Ljava/lang/String;

    move-result-object p0

    invoke-virtual {p2, p0}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    invoke-virtual {p2}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object p0

    invoke-direct {p1, v0, p0}, Lcn/jiguang/ao/n;-><init>(ILjava/lang/String;)V

    return-object p1

    :catch_12f
    move-exception p0

    new-instance p1, Lcn/jiguang/ao/n;

    new-instance p2, Ljava/lang/StringBuilder;

    invoke-direct {p2, p5}, Ljava/lang/StringBuilder;-><init>(Ljava/lang/String;)V

    invoke-virtual {p0}, Ljava/lang/Exception;->getMessage()Ljava/lang/String;

    move-result-object p0

    invoke-virtual {p2, p0}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    invoke-virtual {p2}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object p0

    invoke-direct {p1, v0, p0}, Lcn/jiguang/ao/n;-><init>(ILjava/lang/String;)V

    return-object p1

    :catch_146
    move-exception p0

    new-instance p1, Lcn/jiguang/ao/n;

    new-instance p2, Ljava/lang/StringBuilder;

    const-string p3, "Catch AssertionError to avoid http close crash - "

    invoke-direct {p2, p3}, Ljava/lang/StringBuilder;-><init>(Ljava/lang/String;)V

    invoke-virtual {p0}, Ljava/lang/AssertionError;->getMessage()Ljava/lang/String;

    move-result-object p0

    invoke-virtual {p2, p0}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    invoke-virtual {p2}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object p0

    invoke-direct {p1, v0, p0}, Lcn/jiguang/ao/n;-><init>(ILjava/lang/String;)V

    return-object p1
.end method

.method public static a(Ljava/lang/String;Ljava/lang/String;Landroid/content/Context;ZII)Lcn/jiguang/ao/n;
    .registers 11

    const-string p3, "UTF-8"

    const-string p5, "Exception - "

    const/4 v0, -0x2

    :try_start_5
    invoke-virtual {p1, p3}, Ljava/lang/String;->getBytes(Ljava/lang/String;)[B

    move-result-object p1
    :try_end_9
    .catchall {:try_start_5 .. :try_end_9} :catchall_134
    .catch Ljava/io/UnsupportedEncodingException; {:try_start_5 .. :try_end_9} :catch_13a
    .catch Ljava/lang/AssertionError; {:try_start_5 .. :try_end_9} :catch_138
    .catch Ljava/lang/Exception; {:try_start_5 .. :try_end_9} :catch_136

    :try_start_9
    invoke-static {p1}, Lcn/jiguang/f/k;->a([B)[B

    move-result-object p1
    :try_end_d
    .catchall {:try_start_9 .. :try_end_d} :catchall_134
    .catch Ljava/io/IOException; {:try_start_9 .. :try_end_d} :catch_11b
    .catch Ljava/lang/AssertionError; {:try_start_9 .. :try_end_d} :catch_138
    .catch Ljava/lang/Exception; {:try_start_9 .. :try_end_d} :catch_136

    :try_start_d
    new-instance v1, Lcn/jiguang/net/HttpRequest;

    invoke-direct {v1, p0}, Lcn/jiguang/net/HttpRequest;-><init>(Ljava/lang/String;)V

    const/16 p0, 0x7530

    invoke-virtual {v1, p0}, Lcn/jiguang/net/HttpRequest;->setConnectTimeout(I)V

    invoke-virtual {v1, p0}, Lcn/jiguang/net/HttpRequest;->setReadTimeout(I)V

    const/4 p0, 0x1

    invoke-virtual {v1, p0}, Lcn/jiguang/net/HttpRequest;->setDoOutPut(Z)V

    invoke-virtual {v1, p0}, Lcn/jiguang/net/HttpRequest;->setDoInPut(Z)V

    const/4 v2, 0x0

    invoke-virtual {v1, v2}, Lcn/jiguang/net/HttpRequest;->setUseCaches(Z)V

    invoke-static {}, Lcn/jiguang/bb/c;->a()I

    move-result v3

    int-to-long v3, v3

    invoke-static {v3, v4}, Lcn/jiguang/bb/c;->a(J)Ljava/lang/String;

    move-result-object v3

    const-string v4, "0102030405060708"

    invoke-static {p1, v3, v4, p0}, Lcn/jiguang/bb/c;->a([BLjava/lang/String;Ljava/lang/String;Z)[B

    move-result-object p1

    invoke-virtual {v1, p1}, Lcn/jiguang/net/HttpRequest;->setBody(Ljava/lang/Object;)V

    const-string v4, "Content-Length"

    array-length p1, p1

    invoke-static {p1}, Ljava/lang/String;->valueOf(I)Ljava/lang/String;

    move-result-object p1

    invoke-virtual {v1, v4, p1}, Lcn/jiguang/net/HttpRequest;->setRequestProperty(Ljava/lang/String;Ljava/lang/String;)V

    invoke-virtual {v1, p0}, Lcn/jiguang/net/HttpRequest;->setNeedRetryIfHttpsFailed(Z)V

    sget-object p1, Lcn/jiguang/ao/b;->a:Lcn/jiguang/net/SSLTrustManager;
    :try_end_46
    .catchall {:try_start_d .. :try_end_46} :catchall_134
    .catch Ljava/lang/AssertionError; {:try_start_d .. :try_end_46} :catch_138
    .catch Ljava/lang/Exception; {:try_start_d .. :try_end_46} :catch_136

    if-nez p1, :cond_57

    :try_start_48
    const-string p1, "-----BEGIN CERTIFICATE-----\\n-----END CERTIFICATE-----"

    invoke-static {p1}, Landroid/text/TextUtils;->isEmpty(Ljava/lang/CharSequence;)Z

    move-result v4

    if-nez v4, :cond_57

    new-instance v4, Lcn/jiguang/net/SSLTrustManager;

    invoke-direct {v4, p1}, Lcn/jiguang/net/SSLTrustManager;-><init>(Ljava/lang/String;)V

    sput-object v4, Lcn/jiguang/ao/b;->a:Lcn/jiguang/net/SSLTrustManager;
    :try_end_57
    .catchall {:try_start_48 .. :try_end_57} :catchall_57

    :catchall_57
    :cond_57
    :try_start_57
    sget-object p1, Lcn/jiguang/ao/b;->a:Lcn/jiguang/net/SSLTrustManager;

    if-eqz p1, :cond_60

    sget-object p1, Lcn/jiguang/ao/b;->a:Lcn/jiguang/net/SSLTrustManager;

    invoke-virtual {v1, p1}, Lcn/jiguang/net/HttpRequest;->setSslTrustManager(Lcn/jiguang/net/SSLTrustManager;)V

    :cond_60
    const-string p1, "Accept"

    const-string v4, "application/jason"

    invoke-virtual {v1, p1, v4}, Lcn/jiguang/net/HttpRequest;->setRequestProperty(Ljava/lang/String;Ljava/lang/String;)V

    const-string p1, "Accept-Encoding"

    const-string v4, "gzip"

    invoke-virtual {v1, p1, v4}, Lcn/jiguang/net/HttpRequest;->setRequestProperty(Ljava/lang/String;Ljava/lang/String;)V

    const-string p1, "X-App-Key"

    invoke-static {p2}, Lcn/jiguang/am/a;->a(Landroid/content/Context;)Ljava/lang/String;

    move-result-object v4

    invoke-virtual {v1, p1, v4}, Lcn/jiguang/net/HttpRequest;->setRequestProperty(Ljava/lang/String;Ljava/lang/String;)V

    invoke-virtual {v1, p0}, Lcn/jiguang/net/HttpRequest;->setHaveRspData(Z)V

    invoke-virtual {v1, p0}, Lcn/jiguang/net/HttpRequest;->setRspDatazip(Z)V

    invoke-virtual {v1, p0}, Lcn/jiguang/net/HttpRequest;->setNeedErrorInput(Z)V

    invoke-static {v3}, Lcn/jiguang/ao/k;->a(Ljava/lang/String;)Ljava/lang/String;

    move-result-object p0

    const-string p1, "Authorization"

    invoke-virtual {v1, p1, p0}, Lcn/jiguang/net/HttpRequest;->setRequestProperty(Ljava/lang/String;Ljava/lang/String;)V

    const-string p0, "Charset"

    invoke-virtual {v1, p0, p3}, Lcn/jiguang/net/HttpRequest;->setRequestProperty(Ljava/lang/String;Ljava/lang/String;)V

    :cond_8e
    if-lez p4, :cond_113

    add-int/lit8 p4, p4, -0x1

    invoke-static {p2, v1}, Lcn/jiguang/net/HttpUtils;->httpPost(Landroid/content/Context;Lcn/jiguang/net/HttpRequest;)Lcn/jiguang/net/HttpResponse;

    move-result-object p0

    invoke-virtual {p0}, Lcn/jiguang/net/HttpResponse;->getResponseCode()I

    move-result p1

    const-string p3, "HttpHelper"

    new-instance v3, Ljava/lang/StringBuilder;

    const-string v4, "status code:"

    invoke-direct {v3, v4}, Ljava/lang/StringBuilder;-><init>(Ljava/lang/String;)V

    invoke-virtual {v3, p1}, Ljava/lang/StringBuilder;->append(I)Ljava/lang/StringBuilder;

    const-string v4, " retry left:"

    invoke-virtual {v3, v4}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    invoke-virtual {v3, p4}, Ljava/lang/StringBuilder;->append(I)Ljava/lang/StringBuilder;

    invoke-virtual {v3}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object v3

    invoke-static {p3, v3}, Lcn/jiguang/an/d;->c(Ljava/lang/String;Ljava/lang/String;)V

    const/16 p3, 0xc8

    if-eq p1, p3, :cond_109

    const/16 p3, 0x191

    if-eq p1, p3, :cond_fe

    const/16 p3, 0x194

    const/4 v3, -0x1

    if-eq p1, p3, :cond_f4

    const/16 p3, 0x19a

    if-eq p1, p3, :cond_f4

    const/16 p3, 0x1ad

    if-eq p1, p3, :cond_f4

    const/16 p3, 0x1f7

    if-eq p1, p3, :cond_ea

    const/16 p3, 0xbbd

    if-eq p1, p3, :cond_8e

    const/16 p2, 0x1f4

    if-lt p1, p2, :cond_e0

    new-instance p1, Lcn/jiguang/ao/n;

    invoke-virtual {p0}, Lcn/jiguang/net/HttpResponse;->getResponseBody()Ljava/lang/String;

    move-result-object p0

    invoke-direct {p1, v3, p0}, Lcn/jiguang/ao/n;-><init>(ILjava/lang/String;)V

    return-object p1

    :cond_e0
    new-instance p1, Lcn/jiguang/ao/n;

    invoke-virtual {p0}, Lcn/jiguang/net/HttpResponse;->getResponseBody()Ljava/lang/String;

    move-result-object p0

    invoke-direct {p1, v0, p0}, Lcn/jiguang/ao/n;-><init>(ILjava/lang/String;)V

    return-object p1

    :cond_ea
    new-instance p1, Lcn/jiguang/ao/n;

    invoke-virtual {p0}, Lcn/jiguang/net/HttpResponse;->getResponseBody()Ljava/lang/String;

    move-result-object p0

    invoke-direct {p1, v0, p0}, Lcn/jiguang/ao/n;-><init>(ILjava/lang/String;)V

    return-object p1

    :cond_f4
    new-instance p1, Lcn/jiguang/ao/n;

    invoke-virtual {p0}, Lcn/jiguang/net/HttpResponse;->getResponseBody()Ljava/lang/String;

    move-result-object p0

    invoke-direct {p1, v3, p0}, Lcn/jiguang/ao/n;-><init>(ILjava/lang/String;)V

    return-object p1

    :cond_fe
    new-instance p1, Lcn/jiguang/ao/n;

    const/4 p2, -0x3

    invoke-virtual {p0}, Lcn/jiguang/net/HttpResponse;->getResponseBody()Ljava/lang/String;

    move-result-object p0

    invoke-direct {p1, p2, p0}, Lcn/jiguang/ao/n;-><init>(ILjava/lang/String;)V

    return-object p1

    :cond_109
    new-instance p1, Lcn/jiguang/ao/n;

    invoke-virtual {p0}, Lcn/jiguang/net/HttpResponse;->getResponseBody()Ljava/lang/String;

    move-result-object p0

    invoke-direct {p1, v2, p0}, Lcn/jiguang/ao/n;-><init>(ILjava/lang/String;)V
    :try_end_112
    .catchall {:try_start_57 .. :try_end_112} :catchall_134
    .catch Ljava/lang/AssertionError; {:try_start_57 .. :try_end_112} :catch_138
    .catch Ljava/lang/Exception; {:try_start_57 .. :try_end_112} :catch_136

    return-object p1

    :cond_113
    new-instance p0, Lcn/jiguang/ao/n;

    const-string p1, "Failed - retry enough"

    invoke-direct {p0, v0, p1}, Lcn/jiguang/ao/n;-><init>(ILjava/lang/String;)V

    return-object p0

    :catch_11b
    move-exception p0

    :try_start_11c
    new-instance p1, Lcn/jiguang/ao/n;

    new-instance p2, Ljava/lang/StringBuilder;

    const-string p3, "zip err:"

    invoke-direct {p2, p3}, Ljava/lang/StringBuilder;-><init>(Ljava/lang/String;)V

    invoke-virtual {p0}, Ljava/io/IOException;->getMessage()Ljava/lang/String;

    move-result-object p0

    invoke-virtual {p2, p0}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    invoke-virtual {p2}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object p0

    invoke-direct {p1, v0, p0}, Lcn/jiguang/ao/n;-><init>(ILjava/lang/String;)V

    return-object p1

    :catchall_134
    move-exception p0

    goto :goto_151

    :catch_136
    move-exception p0

    goto :goto_167

    :catch_138
    move-exception p0

    goto :goto_17d

    :catch_13a
    move-exception p0

    new-instance p1, Lcn/jiguang/ao/n;

    new-instance p2, Ljava/lang/StringBuilder;

    invoke-direct {p2, p5}, Ljava/lang/StringBuilder;-><init>(Ljava/lang/String;)V

    invoke-virtual {p0}, Ljava/io/UnsupportedEncodingException;->getMessage()Ljava/lang/String;

    move-result-object p0

    invoke-virtual {p2, p0}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    invoke-virtual {p2}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object p0

    invoke-direct {p1, v0, p0}, Lcn/jiguang/ao/n;-><init>(ILjava/lang/String;)V
    :try_end_150
    .catchall {:try_start_11c .. :try_end_150} :catchall_134
    .catch Ljava/lang/AssertionError; {:try_start_11c .. :try_end_150} :catch_138
    .catch Ljava/lang/Exception; {:try_start_11c .. :try_end_150} :catch_136

    return-object p1

    :goto_151
    new-instance p1, Lcn/jiguang/ao/n;

    new-instance p2, Ljava/lang/StringBuilder;

    invoke-direct {p2, p5}, Ljava/lang/StringBuilder;-><init>(Ljava/lang/String;)V

    invoke-virtual {p0}, Ljava/lang/Throwable;->getMessage()Ljava/lang/String;

    move-result-object p0

    invoke-virtual {p2, p0}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    invoke-virtual {p2}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object p0

    invoke-direct {p1, v0, p0}, Lcn/jiguang/ao/n;-><init>(ILjava/lang/String;)V

    return-object p1

    :goto_167
    new-instance p1, Lcn/jiguang/ao/n;

    new-instance p2, Ljava/lang/StringBuilder;

    invoke-direct {p2, p5}, Ljava/lang/StringBuilder;-><init>(Ljava/lang/String;)V

    invoke-virtual {p0}, Ljava/lang/Exception;->getMessage()Ljava/lang/String;

    move-result-object p0

    invoke-virtual {p2, p0}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    invoke-virtual {p2}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object p0

    invoke-direct {p1, v0, p0}, Lcn/jiguang/ao/n;-><init>(ILjava/lang/String;)V

    return-object p1

    :goto_17d
    new-instance p1, Lcn/jiguang/ao/n;

    new-instance p2, Ljava/lang/StringBuilder;

    const-string p3, "Catch AssertionError to avoid http close crash - "

    invoke-direct {p2, p3}, Ljava/lang/StringBuilder;-><init>(Ljava/lang/String;)V

    invoke-virtual {p0}, Ljava/lang/AssertionError;->getMessage()Ljava/lang/String;

    move-result-object p0

    invoke-virtual {p2, p0}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    invoke-virtual {p2}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object p0

    invoke-direct {p1, v0, p0}, Lcn/jiguang/ao/n;-><init>(ILjava/lang/String;)V

    return-object p1
.end method
